<template>
    <div class="bg-white mt-4">
        <div class="mx-auto max-w-7xl flex flex-col justify-center items-center">
            <h2 class="custom-light-blue my-4 font-semibold">TRANSFORMING THE SME CUSTOMER EXPERIENCE </h2>
            <h1 class="sm:font-semibold text-4xl sm:text-center custom-blue-900">Opening the Doors of Opportunity </h1>
            <p class="text-base md:text-lg custom-grey-dark text-center py-4">Master Loan Origination Challenges with Real Results that can make you ready for<br>
                <span class="font-semibold">Today and the future </span>
                </p>
        </div>
        <div class="mx-auto max-w-7xl grid grid-cols-2 sm:gap-x-8 sm:pt-12 sm:px-8 bg-blue-white">
            <div class="col-span-1 ">
                <img src="/images/LO3.png" alt="Prestatab" class="object-cover object-center group-hover:opacity-75 rounded-lg my-auto">
            </div>

            <div class="col-span-1 flex-col justify-between my-auto">
                <p class=" my-4 tracking-wider text-2xl sm:py-4 custom-light-blue">What’s in it for your <span class="font-semibold">Business ?</span></p>
                <ol  class=" custom-grey-p font-semibold list-decimal list-inside text-base md:text-lg">
                    <li class="sm:py-2">Grow Portfolio & Revenue Per Customer.</li>
                    <li class="sm:py-2">  Streamline Credit Operations.</li>
                    <li class="sm:py-2">  Control & Manage Risk. Increase Productivity – Less Repetitive Tasks.</li>
                    <li class="sm:py-2">  Leverage A Single Platform.</li>
                </ol>




            </div>

        </div>
        <div class="mx-auto max-w-7xl grid grid-cols-2 sm:gap-x-8 sm:px-8 bg-blue-white">
            <div class="col-span-1 flex-col justify-between my-auto">
                <p class=" my-4 tracking-wider text-2xl sm:py-4 custom-light-blue">What’s in it for your <span class="font-semibold">Employees?</span></p>
                <ol  class=" custom-grey-p font-semibold list-decimal list-inside text-base md:text-lg">
                    <li class="sm:py-2">Automated flows – no time-consuming processes.</li>
                    <li class="sm:py-2">Paperless approach, with digitalized forms</li>
                    <li class="sm:py-2">Reduced risk of error – system ensures data consistency</li>
                    <li class="sm:py-2">  Single data entry – no need of entering data multiple times</li>
                </ol>




            </div>
            <div class="col-span-1 ">
                <img src="/images/LO4.png" alt="ladiessitting" class="object-cover object-center group-hover:opacity-75 rounded-lg my-auto">
            </div>



        </div>
        <div class="mx-auto max-w-7xl grid grid-cols-2 sm:gap-x-8 sm:px-8 bg-blue-white">
            <div class="col-span-1 flex-col justify-between items-center my-auto ">
                <img src="/images/LO4.png" alt="ladiessitting" class=" h-1/2 object-cover object-center group-hover:opacity-75 rounded-lg my-auto">
            </div>

            <div class="col-span-1 flex-col justify-between my-auto">
                <p class=" my-4 tracking-wider text-2xl sm:py-4 custom-light-blue">What’s in it for your <span class="font-semibold">Customers?</span></p>
                <ol  class=" custom-grey-p font-semibold list-decimal list-inside text-base md:text-lg">
                    <li class="sm:py-2">Apply Anytime, Anywhere – multichannel approach.</li>
                    <li class="sm:py-2">Complete Digital Document Flow with Digital Approvals.</li>
                    <li class="sm:py-2">Fast pre-qualification and credit decision.</li>
                    <li class="sm:py-2">Decision and fund disbursement within minutes for simple loans.</li>
                </ol>




            </div>

        </div>
    </div>
</template>

<script>
export default {
    name: "Transform.vue"
}
</script>

<style scoped>

.custom-grey-dark{
    color:#656464;
}
.custom-grey-p{
    color:#727272;
}
.custom-blue-900 {

    color: #0A093D
}
.custom-light-blue {
    color: #489AAB
}
</style>
